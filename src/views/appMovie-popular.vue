<template>
    <div class="appMovie-popular">
        <div class="appMovie-popular__row">
            <div
                class="appMovie-popular__col"
                v-for="(value, index) in popularMoviesListOnPage"
                :key="index"
            >
                <div class="appMovie-popular__card">
                    <button
                        class="appMovie-popular__favourites"
                    ...
                    <div class="appMovie-popular__card-genres">
                        <span v-for="(it, idx) in value.genres" :key="idx">{{
                            it.name
                        }}</span>
                    </div>
                </div>
        ...
        <ul class="appMovie-popular__pagination">
            <li
...
export default {
    ...
    computed: {
        ...mapGetters([
            ...
            "detailsMoviesForPopular",
        ]),
        paginationPages() {
            return Math.ceil(
                this.detailsMoviesForPopular.length / this.maxItemOnPage
            );
        },
        popularMoviesListOnPage() {
            let from = (this.currentPageNum - 1) * this.maxItemOnPage;
            let to = from + this.maxItemOnPage;
            return this.detailsMoviesForPopular.slice(from, to);
        },
    },
    methods: {
