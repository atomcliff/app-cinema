<template>
    <div class="appMovie-favourites">
        <div class="appMovie-favourites__row">
            <div
                class="appMovie-favourites__col"
                v-for="(value, index) in fafavouritesMoviesListOnPage"
                :key="index"
            >
                <div class="appMovie-favourites__card">
                    <button
                        class="appMovie-favourites__favourites"
                        @click="addToFavourites(value.id)"
                    >
                        <svg
                    ...
                    <div class="appMovie-favourites__card-genres">
                        <span v-for="(item, idx) in value.genres" :key="idx">{{
                            item.name
                        }}</span>
                    </div>
                </div>
        ...
        <ul class="appMovie-favourites__pagination">
            <li
                v-for="page in paginationPages"
                :key="page"
                @click="togglePaginatPage(page)"
                :class="{ 'pagination-active': page === currentPageNum }"
            >
                {{ page }}
            </li>
        </ul>
...
export default {
    ...
    computed: {
        ...mapGetters(["fafavouritesMoviesList", "fafavouritesMoviesList"]),
        paginationPages() {
            return Math.ceil(
                this.fafavouritesMoviesList.length / this.maxItemOnPage
            );
        },
        fafavouritesMoviesListOnPage() {
            let from = (this.currentPageNum - 1) * this.maxItemOnPage;
            let to = from + this.maxItemOnPage;
            return this.fafavouritesMoviesList.slice(from, to);
        },
    },
    methods: {
        ...mapActions(["add_favourites_movies", "remove_favourites_movies"]),
        togglePaginatPage(page) {
